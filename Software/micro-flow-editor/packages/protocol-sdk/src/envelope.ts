import { CborReader, encodeBytes, encodeMap, encodeUint, ProtocolCodecError } from "./cbor.js";
...
function encodeEnvelope(field1: number, field2: number, payload: Uint8Array): Uint8Array {
  if (payload.length > PAYLOAD_ABSOLUTE_MAX) {
    throw new ProtocolCodecError(`payload of ${payload.length} bytes exceeds the ${PAYLOAD_ABSOLUTE_MAX}-byte limit`);
  }
  return encodeMap([
...
export function encodeRequest(request: RequestEnvelope): Uint8Array {
  if (request.correlationId === 0) throw new ProtocolCodecError("correlationId must be nonzero");
  if (request.operation < 1 || request.operation > 27) {
...
export function encodeResponse(response: ResponseEnvelope): Uint8Array {
  if (response.correlationId === 0) throw new ProtocolCodecError("correlationId must be nonzero");
...
export function encodeEvent(event: EventEnvelope): Uint8Array {
  ...
  if (event.type < 1 || event.type > 4) throw new ProtocolCodecError(`invalid event type ${event.type}`);
  return encodeEnvelope(event.sequence, event.type, event.payload);
}

type RawEnvelope = { readonly field1: number; readonly field2: number; readonly payload: Uint8Array };
...
function decodeEnvelope(bytes: Uint8Array): RawEnvelope {
  ...
  for (const key of map.keys()) {
    if (key < 0 || key > 3) throw new ProtocolCodecError(`unexpected envelope key ${key}`);
  }
  for (const required of [KEY_VERSION, KEY_FIELD1, KEY_FIELD2, KEY_PAYLOAD]) {
    if (!map.has(required)) throw new ProtocolCodecError(`missing envelope key ${required}`);
  }
  ...
  if (field1.kind !== "uint") throw new ProtocolCodecError("envelope field 1 must be a uint");
  if (field1.value === 0n) throw new ProtocolCodecError("envelope field 1 (correlation/sequence) must be nonzero");
  const field2 = map.get(KEY_FIELD2)!;
  if (field2.kind !== "uint") throw new ProtocolCodecError("envelope field 2 must be a uint");
  const payload = map.get(KEY_PAYLOAD)!;
  if (payload.kind !== "bytes") throw new ProtocolCodecError("envelope field 3 (payload) must be a byte string");
  if (payload.value.length > PAYLOAD_ABSOLUTE_MAX) {
    throw new ProtocolCodecError(`payload of ${payload.value.length} bytes exceeds the ${PAYLOAD_ABSOLUTE_MAX}-byte limit`);
  }
  return { field1: Number(field1.value), field2: Number(field2.value), payload: payload.value };
}

export function decodeRequest(bytes: Uint8Array): RequestEnvelope {
  const raw = decodeEnvelope(bytes);
  if (raw.field2 < 1 || raw.field2 > 27) throw new ProtocolCodecError(`unsupported operation ${raw.field2}`);
  return { correlationId: raw.field1, operation: raw.field2 as Operation, payload: raw.payload };
}
...
export function decodeResponse(bytes: Uint8Array): ResponseEnvelope {
  ...
  return { correlationId: raw.field1, status: raw.field2 as ProtocolStatus, payload: raw.payload };
}

export function decodeEvent(bytes: Uint8Array): EventEnvelope {
  const raw = decodeEnvelope(bytes);
  if (raw.field2 < 1 || raw.field2 > 4) throw new ProtocolCodecError(`unsupported event type ${raw.field2}`);
