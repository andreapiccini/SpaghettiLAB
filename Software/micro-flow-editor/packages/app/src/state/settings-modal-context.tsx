...
type SettingsModalContextValue = {
  ...
  closeSettings(): void;
  setCategoryId(categoryId: SettingsCategoryId): void;
};
...
export function SettingsModalProvider({ children }: { readonly children: ReactNode }) {
  ...
  const [categoryId, setCategoryId] = useState<SettingsCategoryId>("general");

  const openSettings = useCallback((next?: SettingsCategoryId) => {
...
export function useSettingsModal(): SettingsModalContextValue {
  ...
  if (!ctx) throw new Error("useSettingsModal() called outside <SettingsModalProvider>");
  return ctx;
}
